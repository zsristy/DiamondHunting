...
(:types
    ...
    (:predicates (avatar_at_position ?x ?y)
                (forward_pos ?x ?y)
                (backward_pos ?x ?y)
                (blocked ?x ?y)
(door_lock ?x ?y)
    ...
    (:action unlock_door
        ...
        :precondition (and (door_at ?d ?x1 ?y1) (door_lock ?x1 ?y1)(not(door_opened ?d)) (avatar_at_position ?x1 ?y1) (lock_key_pair ?d ?k) (key_taken ?k))
